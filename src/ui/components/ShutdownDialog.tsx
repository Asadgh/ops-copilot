  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(720px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-lg border border-oc-border/70 bg-oc-surface shadow-2xl shadow-black/35">
            ...
            <div className="grid gap-3 p-4">
              {prompts.map((prompt) => (
                <label key={prompt.key} className="text-xs font-medium text-oc-muted">
                  ...
                  <textarea
                    className="oc-textarea min-h-20 w-full px-3 py-2 text-sm leading-5 text-oc-text placeholder:text-oc-muted"
