...
export function ReminderDialog({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50 bg-black/58 backdrop-blur-sm" />
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(460px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-lg border border-oc-border/70 bg-oc-surface shadow-2xl shadow-black/35">
          <form onSubmit={submit}>
            ...
            <div className="grid gap-3 p-4">
              <label className="text-xs font-medium text-oc-muted">
                ...
                <input className="oc-input h-9 w-full px-3 text-sm text-oc-text" value={title} onChange={(event) => setTitle(event.target.value)} />
              </label>
              <label className="text-xs font-medium text-oc-muted">
                <span className="mb-1.5 block">Due At</span>
                <input className="oc-input h-9 w-full px-3 text-sm text-oc-text" type="datetime-local" value={dueAt} onChange={(event) => setDueAt(event.target.value)} />
              </label>
            </div>
